...
<body>
	...
	<ul>
		<li>파라미터를 받아서 원하는 처리를 수행한 후
			결과를 저장해 놓을 수 있는 곳</li>
	...
	<ul>
		...
		<li>요청에 대한 응답을 수행하고나면 저장한 데이터가 모두 사라진다</li>
	...
	<ul>
		<li>하나의 세션이 만료될때까지 수명이 유지되는 어트리뷰트</li>
		<li>서버는 해당 서버에 최초로 요청을 보낸 클라이언트에게 세션ID를 부여한다</li>
		<li>발급받은 세션ID는 서버에서 유일한 값이며 
			해당 클라이언트 본인임을 증명 할 수 있는 식별값이 된다</li>
		<li>세션은 해당 세션ID가 유효한 기간동안 수명이 유지된다</li>
	...
	<ul>
		...
		<li>세션 객체는 세션ID의 개수만큼(즉, 접속자 수만큼)의 인스턴스가 존재하지만
			서버는 1대이므로 전역 변수 같은 역할을 하게 된다</li>
	</ul>
	...
	<a href="/chap03/attribute/sample/addAllAttr.jsp">모든 영역에 어트리뷰트 추가해보기 (리다이렉트)</a>
	<a href="/chap03/attribute/sample/addAllAttr2.jsp">모든 영역에 어트리뷰트 추가해보기 (포워드)</a>
