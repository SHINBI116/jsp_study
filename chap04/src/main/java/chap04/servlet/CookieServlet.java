...
public class CookieServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.printf("URI가 %s일때, cmd는 %s입니다.\n", uri, cmd);

		if (cmd.equals("list")) {

			// 클라이언트가 보유하고 있는 쿠키는 모든 요청에 함께 실려오게 된다 (경로만 맞다면)
			// 쿠키를 하나만 꺼내는 메서드가 기본적으로 구현되어있지 않다
			...
			// 새로운 쿠키를 응답에 함께 실어보내면 응답을 받은 웹 브라우저에 추가된다
			...
			cookie2.setMaxAge(5); // 수명이 5초 밖에 안되는 쿠키
			...
			if (cookies != null) {
				...
				for (Cookie cookie : cookies) {
					if (cookie.getName().equals(to_delete)) {
						...
						cookie.setMaxAge(0);
						// 이름만 맞다고 삭제되는게 아니라 경로까지 맞아야 해당 쿠키를 정확하게
						// 지칭하는 것이 된다
						cookie.setPath("/chap04/cookie/");
						resp.addCookie(cookie);
						break;
			...
			// 이 서블릿의 list 커맨드로 다시 요청 해주세요~ 라고 응답
			...
			System.out.println("바꿔야하는 쿠키이름 : " + to_modi);
